...
const TextEditor = ({ item }: Props) => {
  ...
  const update = (patch: Partial<ITextTrackItem>) => {
    updateTrackItemById(item.id, patch)
    eventBus.emit('text:update', { id: item.id, patch })
  }
  ...
  return (
    <div className="material-editor__fields">
      ...
      <Section title="字体" />
      <FieldRow label="大小">
        <InputNumber size="small" min={8} max={200} value={item.fontSize ?? 48}
          onChange={(v) => update({ fontSize: v ?? 48 })} style={{ width: '100%' }} />
      </FieldRow>
      <FieldRow label="字体">
        <Select
          size="small"
