...
import type { ISourceNode } from '@van-gogh/video-render-constants'
import { type INode, timeConfig } from '@van-gogh/video-render-constants'
...
export const getBufferImage = (editor: Editor, node: ISourceNode, canvasEl: HTMLCanvasElement) => {
  if(node?.imageData) {
  ...
  try {
    let { width, height } = node.pipParams
    if (editor.imageCatch[`imageCatch${node.id}`]) {
      ...
      if (width > editor.canvasWidth || height > editor.canvasHeight) {
        ...
        width = obj.width
        height = obj.height
        node.pipParams.width = obj.width
        node.pipParams.height = obj.height
      }
...
export const getGifImage = (editor: Editor, node: ISourceNode, canvasEl: HTMLCanvasElement) => {
  ...
  const time: number = node._currentTime * timeConfig.MILL_TIME_CONVERSION
  // 按照每秒钟25帧处理，获取当前帧数
  let frameInx: number = Math.round(time / timeConfig.FRAME_TIME_MILL) % frameCount
  if (!frameInx) {
    frameInx = 1
  } // 因为gif第一帧都没有画面
